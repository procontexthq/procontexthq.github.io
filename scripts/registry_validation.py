...
def validate_libraries(libraries: list[Any]) -> list[ValidationError]:
    ...
    seen_packages: dict[str, dict[str, str]] = {}
    ...
    for i, entry in enumerate(libraries):
        ...
        for j, pkg_entry in enumerate(packages):
            ...
            if not isinstance(pkg_names, list) or not all(isinstance(p, str) for p in pkg_names):
                errors.append(ValidationError(11, display_id, f"{pkg_label}.package_names must be a list of strings"))
                pkg_names = []

            langs = pkg_entry.get("languages")
            if langs is not None:
                if not isinstance(langs, list) or not all(isinstance(l, str) for l in langs):
                    errors.append(ValidationError(12, display_id, f"{pkg_label}.languages must be a list of strings"))
...
def append_failed_url_entries(libraries: list[Any], url_errors: list[ValidationError]) -> None:
    ...
    if not new_entries:
        print(f"  (all failing entries already recorded in {display_path(FAILED_URLS_FILE)})")
        return
    ...
    print(
        ...
        f"appended to {display_path(FAILED_URLS_FILE)}"
    )
...
def update_metadata(libraries_checksum: str, additional_info_checksum: str, metadata_path: Path) -> None:
    with open(metadata_path) as f:
        meta = json.load(f)
    new_version = bump_version(meta.get("version", ""))
    ...
    print(f"  written to: {display_path(metadata_path)}")


def collect_libraries_errors(
    libraries_file: Path,
    *,
